...
if [ "$device_count" -gt 1 ]; then
    selected_device=$(echo "$devices" | rofi -dmenu -config ~/.config/rofi/config.rasi -p "Select Device: ")
else
...
case "$selected_option" in
    "Video")
      scrcpy --serial "$selected_device" --max-size=1024 --video-codec=h265 --video-bit-rate=6M --audio-bit-rate=128K --max-fps=30 --no-audio --turn-screen-off
        ;;
    "No Video")
